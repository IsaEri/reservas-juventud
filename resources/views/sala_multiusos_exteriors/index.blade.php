...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card">
                 <div class="card-header"><b>Sala multiusos exterior</b>
                 @if( Auth::user()->is_admin == 1) 
                 <a href='{{route('sala_multiusos_exteriors.create')}}' class="btn btn-success btn-sm">Crear reserva</a>
                 <br><br>
                 <!-- Buscador -->
                 {!! Form::open([ 'route' => 'sala_multiusos_exteriors.index', 'method' => 'GET', 'class' => 'navbar-form pull-right']) !!}
                ...
                <div class="card-body">
                    ...
                    @foreach($sala_multiusos_exteriors as $socio)
                        <tr>
                            ...
                            @if( Auth::user()->is_admin == 1) 
                            <td><a href='{{route('sala_multiusos_exteriors.edit',['id'=>$socio->id])}}' class="btn btn-success btn-sm">Editar</a></td>
                            <td>
                                <form method="post" action="{{route('sala_multiusos_exteriors.destroy',['id'=>$socio->id])}}">
                                    @csrf
                                    {!! method_field('delete') !!}
...
@endif
                     ...
                     {{ $sala_multiusos_exteriors -> links() }}
                </div>
            </div>
        </div>
    </div>
